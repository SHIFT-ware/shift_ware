...
# Serverspecが全てNG
echo 01
grep -E "OK=0," Shift_Log/Shift.6/Serverspec_Result_192.168.127.31.csv
result01=$?

echo 02
grep -E "OK=0," Shift_Log/Shift.6/Serverspec_Result_192.168.127.41.csv
result02=$?


# Ansibleの結果判定
echo 03
grep -E "192\.168\.127\.31\s*:\s*ok=[0-9]{1,3}\s*changed=[0-9]{1,3}\s*unreachable=0\s*failed=0" Shift_Log/Shift.5/Ansible.log
result03=$?

echo 04
grep -E "192\.168\.127\.41\s*:\s*ok=[0-9]{1,3}\s*changed=[0-9]{1,3}\s*unreachable=0\s*failed=0" Shift_Log/Shift.5/Ansible.log
result04=$?
...
# Ansibleの結果判定
echo 07
grep -E "192\.168\.127\.31\s*:\s*ok=[0-9]{1,3}\s*changed=0\s*unreachable=0\s*failed=0" Shift_Log/Shift.3/Ansible.log
result07=$?

echo 08
grep -E "192\.168\.127\.41\s*:\s*ok=[0-9]{1,3}\s*changed=0\s*unreachable=0\s*failed=0" Shift_Log/Shift.3/Ansible.log
result08=$?
