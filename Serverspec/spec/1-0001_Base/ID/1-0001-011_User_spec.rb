describe "011_User" do
  context "ユーザー" do
	...
	user_resource = user(user[:name])
	expect(user_resource).to exist
      end

      it "#{user[:name]}のuidが#{user[:uid]}であること", if: user.has_key?(:uid) do
	user_resource = user(user[:name])
	expect(user_resource).to have_uid user[:uid]
      end

      it "#{user[:name]}が#{user[:group]}に所属すること", if: user.has_key?(:group) do
	user_resource = user(user[:name])
	expect(user_resource).to belong_to_primary_group user[:group]
      end

      it "#{user[:name]}のホームディレクトリが#{user[:home_dir]}であること", if: user.has_key?(:home_dir) do
	user_resource = user(user[:name])
	expect(user_resource).to have_home_directory user[:home_dir]
      end

      it "#{user[:name]}のログインシェルが#{user[:shell]}であること", if: user.has_key?(:shell) do
	user_resource = user(user[:name])
	expect(user_resource).to have_login_shell user[:shell]
      end
